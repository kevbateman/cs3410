...
func (elt *Replica) Prepare(req Request, resp *Response) error {
	...
	args := req.Prepare
	chat("")
	chat("- Prepare:  Args(SLOT: " + strconv.Itoa(args.Slot.Index) + ", SEQUENCE: " + strconv.Itoa(args.Seq.Number) + ")")
	chat("- Prepare:  " + args.Slot.String())

	if elt.Slots[args.Slot.Index].Sequence.Cmp(args.Seq) == -1 {
		//if elt.Promised < args.Seq
		chat("- Prepare:  Sequence YES")
		resp.Okay = true
		...
		resp.Okay = false
		chat("- Prepare:  Sequence NO. Already promised a higher number: " + strconv.Itoa(elt.Slots[args.Slot.Index].Sequence.Number))
...
func (elt *Replica) Accepted(req Request, resp *Response) error {
	...
	args := req.Accepted
	chat("")
	chat("- Accepted:  Args(SLOT: " + strconv.Itoa(args.Slot.Index) + ", SEQUENCE: " + strconv.Itoa(args.Seq.Number) + ")")
	chat("- Accepted:  " + args.Slot.String())

	if elt.Slots[args.Slot.Index].Sequence.Cmp(args.Seq) == 0 {
		chat("- Accepted:  Sequence YES")
		resp.Okay = true
		resp.Promised = elt.Slots[args.Slot.Index].Sequence
		elt.ToApply = args.Slot
	} else {
		chat("- Accepted:  Sequence NO. Already promised a higher number: " + strconv.Itoa(elt.Slots[args.Slot.Index].Sequence.Number))
		resp.Okay = false
...
func (elt *Replica) Decide(req Request, resp *Response) error {
	...
	args := req.Decide
	chat("")
	chat("> Decide:  Args(SLOT: " + strconv.Itoa(args.Slot.Index) + ", VALUE: " + args.Value.Command + ")")
	chat("> Decide:  " + args.Slot.String())

	if elt.Slots[args.Slot.Index].Decided && elt.Slots[args.Slot.Index].Command.Command != args.Value.Command {
		chat("> Decide:  Already decided slot " + strconv.Itoa(args.Slot.Index) + " with a different command " + args.Value.Command)
		log.Fatal("Decide")
		return nil
	}
	// If already decided, quit
	if elt.Slots[args.Slot.Index].Decided {
		chat("> Decide:  Already decided slot " + strconv.Itoa(args.Slot.Index) + " with command " + args.Value.Command)
		return nil
	}
...
rounds:
	for {
		...
		chat("* Propose:  Round: " + strconv.Itoa(round))
		chat("* Propose:  Slot: " + strconv.Itoa(pSlot.Index))
		chat("* Propose:  N: " + strconv.Itoa(pSlot.Sequence.Number))
...
func (elt *Replica) Accept(req Request, resp *Response) error {
	...
	chat("")
	chat("* Accept:  Args(COMMAND: " + aV.String() + ", SEQUENCE: " + aN.String() + ", SLOT: " + aSlot.String() + ")")
	...
	if nTrue >= majority(len(elt.Cell)) {
		chat("* Accept:  Received enough votes, you can now decide.")
		for _, v := range elt.Cell {
	...
	chat("* Accept:  Not enough votes.  You can't decide. Start over.")
...
func main() {
	...
	for scanner.Scan() {
		...
		case "get":
			...
			go func() {
				chat("Finished " + <-node.Listeners[key])
			}()
			break
		case "put":
			var command Command
